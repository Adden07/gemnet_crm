...
class InvoiceTaxFbrExport implements FromCollection,WithHeadings, WithMapping
{
    ...
    {
        return Invoice::with(['user'])->where('adv_inc_tax', '>', 0)->where('tax_paid', 0)->whereYear('created_at', date('Y',strtotime($this->date)))->whereMonth('created_at', date('m',strtotime($this->date)))->get();
    }
